...
public class e_MessagesActivity extends Activity {
  ...
  private class fetchAllMessages_Task extends AsyncTask<Integer, Void, List<Message>> {
    ...
    @Override
    protected List<Message> doInBackground(Integer... userIds) { //list userIds
      try {
    ...
    protected void onPostExecute(List<Message> all_messages) {
      ...
      if (all_messages == null) {
        ...
        // Feed adapter view with ListMessages
        adapter = new MyAdapter_messages(e_MessagesActivity.this, messages, globalState.my_user);
  ...
  private class fetchNewMessages_Task extends AsyncTask<Integer, Void, List<Message>> {
    ...
    protected List<Message> doInBackground(Integer... userIds) {
      ...
      try {
        ...
        if (!messages.isEmpty()) { // Si no es null
  ...
  public void sendText(final View view) {
    ...
    Date date = new Date();
    newmes.setContent(input_text.getText().toString());
    newmes.setDate(date);
    newmes.setUserReceiver(globalState.user_to_talk_to);
    newmes.setUserSender(globalState.my_user);
    //newmes.setId(4); //TEST
    //RPC.postMessage(newmes);
      new SendMessage_Task().execute(newmes);
    input_text.setText("");
  ...
  private class SendMessage_Task extends AsyncTask<Message, Void, Boolean> {
    ...
    protected void onPostExecute(Boolean resultOk) {
      if (resultOk) {
        ...
        /*List<Message> messToUpdate = RPC.retrieveNewMessages(globalState.my_user.getId(),globalState.user_to_talk_to.getId(),adapter.getLastMessage());
        if (messToUpdate.size()>1){
          adapter.addMessages(messToUpdate);
        }else{
          adapter.addMessage(messToUpdate.get(0));
        }
        conversation.setAdapter(adapter);
        */
        conversation.post(new Runnable() {
