...
public class RPC {
  ...
  public static List<Message> retrieveMessages(int from, int to) { //passas ID de usuaris
    try {
  ...
  public static List<Message> retrieveNewMessages(int from, int to, Message message) {
    try {
      ...
      PrintWriter out = new PrintWriter(ucon.getOutputStream(), true); //envies 1 missatge al server, ell agafara els temps i tenviara llista en funcio del temps
      out.println(gson.toJson(message));
